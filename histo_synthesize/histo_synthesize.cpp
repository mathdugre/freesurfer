...
static int prune_indices(
    MRI *mri_mask, MRI *histo, short *xind, short *yind, short *zind, int nind, int use_val, float hthresh);
int main(int argc, char *argv[]);
static int get_option(int argc, char *argv[]);
MRI *HISTOsynthesize(MRI *mri,
                     MRI *histo,
                     int test_slice,
                     int train_slice,
                     MRI *hsynth,
...
static int find_most_similar_location(MRI *mri,
                                      FEATURE *fsrc,
                                      int z,
                                      int *pxd,
                                      int *pyd,
...
int main(int argc, char *argv[])
{
  ...
  mri = MRIread(argv[1]);
  if (mri == NULL) ErrorExit(ERROR_NOFILE, "%s: could not read MRI volume %s", Progname, argv[1]);
  if (downsample > 0) {
    MRI *mri_tmp;
    int n;
    ...
    for (n = 0; n < downsample; n++) {
      ...
      mri = mri_tmp;
      train_slice /= 2;
      if (flags & MRI_SPACE) test_slice /= 2;
    }
  }
  histo = MRIread(argv[2]);
  if (histo == NULL) ErrorExit(ERROR_NOFILE, "%s: could not read histological volume %s", Progname, argv[2]);
  if (training_src_fname) {
    ...
    if (train_slice >= mri_train_dst->depth) train_slice = MAX(0, (mri_train_dst->depth - 1) / 2);
  }
  else {
    if (train_slice >= histo->depth) train_slice = MAX(0, (histo->depth - 1) / 2);
  }
  if (test_slice >= mri->depth) test_slice = (mri->depth - 1) / 2;
  hsynth =
      HISTOsynthesize(mri, histo, test_slice, train_slice, NULL, wsize, flags, argv[3], mri_train_src, mri_train_dst);
...
static void usage_exit(int code)
{
  printf("usage: %s [options] <MRI volume> <HISTO volume> <synthetic histo>", Progname);
  printf("\t\n");
  exit(code);
}
                     ...
                     MRI *mri_train_dst)
{
  ...
  if (mri_train_src == NULL) mri_train_src = mri;
  if (mri_train_dst == NULL) mri_train_dst = histo;
  ...
  if (identity) printf("image geometries are identical - disabling transforms\n");
    ...
    for (y = 0; y < mri_train_src->height; y++) {
      for (z = 0; z < mri_train_src->depth; z++) {
        ...
        if ((val < 240) && (val != 4080) && MRIindexNotInVolume(mri_train_dst, xh, yh, zh) == 0) {
          ...
          if (x < box_train.x) box_train.x = x;
          if (y < box_train.y) box_train.y = y;
          if (z < box_train.z) box_train.z = z;
          if (x > x1) x1 = x;
          if (y > y1) y1 = y;
          if (z > z1) z1 = z;
  ...
  if (mri_train_src != mri) MRIhistogramNormalize(mri_train_src, mri, mri_train_src);
  ...
  if (flags & MRI_SPACE) {
    ...
    for (x = 0; x < mri->width; x++) {
      for (y = 0; y < mri->height; y++) {
        ...
        if ((val != 4080) && MRIindexNotInVolume(histo, xh, yh, zh) == 0) {
          ...
          if (x < box_test.x) box_test.x = x;
          if (y < box_test.y) box_test.y = y;
          if (z < box_test.z) box_test.z = z;
          if (x > x1) x1 = x;
          if (y > y1) y1 = y;
          if (z > z1) z1 = z;
    ...
    for (x = box_test.x; x < box_test.x + box_test.dx; x++) {
      ...
      for (y = box_test.y; y < box_test.y + box_test.dy; y++) {
        if (x == Gx && y == Gy && test_slice == Gz) DiagBreak();
        if (MRIgetVoxVal(mri_mask, x, y, test_slice, 0) == 0) continue;
        V3_X(v1) = x;
        V3_Y(v1) = y;
        V3_Z(v1) = test_slice;
        extract_feature_vector(mri, x, y, test_slice, f);
        find_most_similar_location(mri,
                                   f,
                                   train_slice,
                                   &xd,
                                   &yd,
...
#ifdef HAVE_OPENMP
        xm = ym = zm = 0;
#pragma omp parallel for if_ROMP(experimental) firstprivate(m_histo2mri,           \
                                                                m_mri2histo,       \
                                                                xind,              \
                                                                yind,              \
                                                                zind,              \
                                                                ...
                                                                zd,                \
                                                                f,                 \
                                                                farray) shared(v1, v2) schedule(dynamic, 1)
#endif
    for (x = 0; x < width; x++)  // synth is in histo coords
    {
      ...
      for (y = 0; y < hsynth->height; y++) {
        ...
        find_most_similar_location(mri_train_src,
                                   f,
                                   train_slice,
                                   &xd,
                                   &yd,
  ...
  for (x = 0; x < _MAX_FS_THREADS; x++) free(farray[x].vals);
  ...
  for (i = 0, xk = -f->whalf; xk <= f->whalf; xk++) {
    ...
    for (yk = -f->whalf; yk <= f->whalf; yk++) {
      ...
      for (zk = -f->whalf; zk <= f->whalf; zk++, i++) {
        ...
        if (f->flags & SUBTRACT_CENTER) f->vals[i] -= val;
      }
    }
  }
...
#pragma omp parallel for if_ROMP(experimental)                                                                       \
    firstprivate(nind, x, y, x1, y1, Gx, Gy, Gz, box, x0, y0, fsrc, f, flags, min_training_dist, dist, num_notfound) \
    shared(min_dist, num) schedule(dynamic, 1)
#endif
#endif
  for (ind = 0; ind < nind; ind++) {
    ...
    x = xind[ind];
    y = yind[ind];
    if (x < box->x || x > x1 || y < box->y || y > y1) continue;
    if (x == Gx && y == Gy && z == Gz) DiagBreak();
    if (MRIgetVoxVal(mri_mask, x, y, z, 0) == 0) continue;
    if ((mri->depth == 1) && ((abs(x - x0) < min_training_dist) || (abs(y - y0) < min_training_dist)))
  ...
  switch (which) {
    ...
    case L2_NORM_DIST:
      for (i = 0; i < f1->len; i++) {
        v1 = f1->vals[i];
        v2 = f2->vals[i];
        if (FZERO(v1) || FZERO(v2) || FEQUAL(v1, 255) || FEQUAL(v2, 255)) continue;
        num++;
        sse += SQR(v1 - v2);
      }
      sse = sqrt(sse / num);
...
static int dump_window(MRI *mri, const char *fname, int x0, int y0, int z0, int wsize)
{
  ...
  fp = fopen(fname, "w");
  if (fp == NULL) ErrorReturn(ERROR_NOFILE, (ERROR_NOFILE, "dump_window(%s): could not open file", fname));
  whalf = (wsize - 1) / 2;
