...
static const char *single_classifier_names[] = {"NOT WMSA", "WMSA", "FUTURE WMSA"};
...
static RANDOM_FOREST *train_rforest(MRI *mri_inputs[MAX_SUBJECTS][MAX_TIMEPOINTS],
...
int main(int argc, char *argv[])
{
  ...
  if (parms.nvols == 0) parms.nvols = ninputs;
  ...
  if (gca == NULL) ErrorExit(ERROR_NOFILE, "%s: could not read GCA from %s", Progname, gca_name);
...
#pragma omp parallel for if_ROMP(experimental) firstprivate(tp1_name,                                                 \
                                                                tp2_name,                                             \
                                                                mri_in,                                               \
                                                                mri_tmp,                                              \
                                                                input,                                                \
                                                                ...
                                                                sname,                                                \
                                                                t,                                                    \
                                                                fname) shared(mri_inputs, transforms, mri_segs, argv) \
    schedule(dynamic, 1)
#endif
  for (i = 0; i < max_index; i++) {
    ...
    printf(
        ...
        "************************************\n");
    printf(
        "processing subject %s, %d of %d (%s and %s)...\n", subject_name, i + 1 - nargs, nsubjects, s1_name, s2_name);
    ...
    for (t = 0; t < 2; t++) {
      ...
      if (Gdiag & DIAG_SHOW && DIAG_VERBOSE_ON) fprintf(stderr, "Reading segmentation from %s...\n", fname);
      mri_seg = MRIread(fname);
      if (!mri_seg) ErrorExit(ERROR_NOFILE, "%s: could not read segmentation file %s", Progname, fname);
      ...
      if (wmsa_fname) {
        ...
        if (mri_wmsa == NULL) ErrorExit(ERROR_NOFILE, "%s: could not read WMSA file %s", fname);
        ...
        if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) {
          char s[STRLEN];
          int req = snprintf(s, STRLEN, "%s/%s/mri/seg_%s", subjects_dir, subject_name, wmsa_fname);
          if (req >= STRLEN) {
            std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__ << std::endl;
          }
          MRIwrite(mri_seg, s);
      ...
      if (insert_fname) {
        ...
        int req = snprintf(fname, STRLEN, "%s/%s/mri/%s", subjects_dir, subject_name, insert_fname);
        if (req >= STRLEN) {
          std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__ << std::endl;
        }
        mri_insert = MRIread(fname);
        if (mri_insert == NULL)
          ErrorExit(ERROR_NOFILE, "%s: could not read volume from %s for insertion", Progname, insert_fname);
      ...
      ////////////////////////////////////////////////////////////
      if (DIAG_VERBOSE_ON) fprintf(stderr, "Gather all input volumes for the subject %s.\n", subject_name);
      ...
      for (input = 0; input < ninputs; input++) {
        ...
        int req = snprintf(fname, STRLEN, "%s/%s/mri/%s", subjects_dir, sname, input_names[input]);
        if (req >= STRLEN) {
          std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__ << std::endl;
        }
        if (DIAG_VERBOSE_ON) printf("reading co-registered input from %s...\n", fname);
        fprintf(stderr, "   reading input %d: %s\n", input, fname);
        mri_tmp = MRIread(fname);
        if (!mri_tmp) ErrorExit(ERROR_NOFILE, "%s: could not read image from file %s", Progname, fname);
        // input check 1
        if (getSliceDirection(mri_tmp) != MRI_CORONAL) {
          ErrorExit(ERROR_BADPARM, "%s: must be in coronal direction, but it is not\n", fname);
        }
        // input check 2
        if (conform && (mri_tmp->xsize != 1 || mri_tmp->ysize != 1 || mri_tmp->zsize != 1)) {
        ...
        else if ((force_inputs == 0) && (!FEQUAL(TRs[input], mri_tmp->tr) || !FEQUAL(FAs[input], mri_tmp->flip_angle) ||
        ...
        if (input == 0) {
          ...
          ///////////////////////////////////////////////////////////
          mri_in = MRIallocSequence(mri_tmp->width, mri_tmp->height, mri_tmp->depth, mri_tmp->type, nframes);
          if (!mri_in)
        ...
        if (mask_fname) {
          ...
          if (!mri_mask) ErrorExit(ERROR_NOFILE, "%s: could not open mask volume %s.\n", Progname, fname);
      ...
      if (xform_name) {
        ...
        int req = snprintf(fname, STRLEN, "%s/%s/mri/transforms/%s", subjects_dir, sname, xform_name);
        if (req >= STRLEN) {
          std::cerr << __FUNCTION__ << ": Truncation on line " << __LINE__ << std::endl;
        }
        if (Gdiag & DIAG_SHOW && DIAG_VERBOSE_ON) printf("INFO: reading transform file %s...\n", fname);
        if (!FileExists(fname)) {
          fprintf(stderr, "ERROR: cannot find transform file %s\n", fname);
          exit(1);
        }
        transform = TransformRead(fname);
        if (!transform) ErrorExit(ERROR_NOFILE, "%s: could not read transform from file %s", Progname, fname);
  ...
  if (RFwrite(rf, out_fname) != NO_ERROR) ErrorExit(ERROR_BADFILE, "%s: could not write rf to %s", Progname, out_fname);
...
static int get_option(int argc, char *argv[])
{
  ...
  if (!stricmp(option, "GRADIENT")) {
    ...
    if (ctab == NULL) ErrorExit(ERROR_NOFILE, "%s: could not read color table from %s", Progname, argv[2]);
    nargs = 1;
  }
  ...
  else if (!stricmp(option, "PARC_DIR") || !stricmp(option, "SEG_DIR") || !stricmp(option, "SEG") ||
    ...
    do_fix_badsubjs = 1;
    printf(
        "will conduct sanity-check of labels and write corrected "
    ...
    if (smooth <= 0 || smooth > 1)
      ErrorExit(ERROR_BADPARM, "%s: smoothing parameter %2.1f must be in [0,1]\n", Progname, smooth);
    nargs = 1;
...
static void usage_exit(int code)
{
  ...
  printf(
      "Usage  : %s [options] <subject1> <subject2> ... "
      "<output rfa fname>\n",
      Progname);
  printf(
...
static int input_labels[] = {
    Left_Cerebral_Exterior, Right_Cerebral_Exterior, Left_Cerebellum_Exterior, Right_Cerebellum_Exterior};
// replace the values above with the following
static int output_labels[] = {
    Left_Cerebral_Cortex, Right_Cerebral_Cortex, Left_Cerebellum_Cortex, Right_Cerebellum_Cortex};
  ...
  for (x = 0; x < mri_seg->width; x++) {
    for (y = 0; y < mri_seg->height; y++) {
      for (z = 0; z < mri_seg->depth; z++) {
        ...
        if (label != WM_hypointensities) continue;
    ...
    for (y = 0; y < mri_seg->height; y++) {
      for (z = 0; z < mri_seg->depth; z++) {
        ...
        if (label != Unknown) {
          ...
          // labelled as Unknown.
          if ((x < 6) || (y < 6) || (z < 6) || (x > 249) || (y > 249) || (z > 249)) {
            printf(
        ...
        if ((label == Left_Hippocampus) || (label == Right_Hippocampus) || (label == Left_Caudate) ||
            (label == Right_Caudate) || (label == Left_Amygdala) || (label == Right_Amygdala) ||
            (label == Left_Putamen) || (label == Right_Putamen) || (label == Left_Pallidum) ||
            (label == Right_Pallidum) || (label == Left_Inf_Lat_Vent) || (label == Right_Inf_Lat_Vent)) {
          ...
          transform_point(mri_seg->linear_transform, xw, yw, zw, &xmt, &ymt, &zmt);  // get mni tal coords
                                    ...
                                    int ntp)
{
  ...
  */
  ...
  rf = RFalloc(
      parms->ntrees, nfeatures, NCLASSES, parms->max_depth, const_cast< char ** >(single_classifier_names), max_steps);
  if (rf == NULL) ErrorExit(ERROR_NOFILE, "%s: could not allocate random forest", Progname);
  rf->min_step_size = 1;
      ...
      for (y = 0; y < mri_in->height; y++)
        for (z = 0; z < mri_in->depth; z++)
          if (is_possible_wmsa(gca, mri_in, transform, x, y, z, 0)) {
            ...
            if (xt == Gx && yt == Gy && zt == Gz) DiagBreak();
            MRIsetVoxVal(mri_wmsa_possible, xt, yt, zt, 0, 1);
          }
  }
  for (; wmsa_whalf > 0; wmsa_whalf--) MRIdilate(mri_wmsa_possible, mri_wmsa_possible);
    ...
    for (x = 0; x < mri_in->width; x++)
      for (y = 0; y < mri_in->height; y++)
        for (z = 0; z < mri_in->depth; z++) {
          ...
          zt = nint(zatlas / tvoxel_size);
          if (xt == Gx && yt == Gy && zt == Gz) DiagBreak();
          if ((IS_WMSA(label) == 0) && MRIgetVoxVal(mri_wmsa_possible, xt, yt, zt, 0) == 0) continue;
          if (NOT_TRAINING_LABEL(label)) continue;
          ntraining++;
  ...
  if (correct != ntraining) DiagBreak();
  ...
  printf("initial training set found with %dK FUTURE WMSA labels, %dK WMSA labels, and %dK non (ratio=%2.1f:%2.1f)\n",
         ...
         nwmsa / 1000,
         nnot / 1000,
         (float)nnot / (float)nfuture,
         (float)nnot / (float)nwmsa);
  ...
  for (i = n = 0; n < nsubjects; n++) {
    ...
    for (x = 0; x < mri_in->width; x++)
      for (y = 0; y < mri_in->height; y++)
        for (z = 0; z < mri_in->depth; z++) {
          if ((int)MRIgetVoxVal(mri_training_voxels, x, y, z, n) == 0) continue;
          label = MRIgetVoxVal(mri_seg, x, y, z, 0);
          TransformSourceVoxelToAtlas(transform, mri_in, x, y, z, &xatlas, &yatlas, &zatlas);
          xt = nint(xatlas / tvoxel_size);
          yt = nint(yatlas / tvoxel_size);
          ...
          if (training_data[i] == NULL)
            ErrorExit(ERROR_NOMEMORY, "train_rforest: could not allocate %d-len feature vector #%d", nfeatures, i);
          ...
          if (training_data[i][Gdiag_no] < 80 && training_classes[i] == 1) DiagBreak();
          i++;
        }
    MRIfree(&mri_in);
    MRIfree(&mri_seg);
  ...
  printf("out of bag accuracy: %d of %d = %2.2f%%\n", correct, ntraining, 100.0 * correct / ntraining);
  if (log_file_name) {
    struct flock fl;
    int fd;
    ...
    if (fd < 0) ErrorExit(ERROR_NOFILE, "%s: could not open test log file %s", Progname, log_file_name);

    fcntl(fd, F_SETLKW, &fl); /* F_GETLK, F_SETLK, F_SETLKW */
    sprintf(line, "%f %d %d %f\n", rf->training_fraction, rf->max_depth, rf->ntrees, 100.0 * correct / ntraining);
