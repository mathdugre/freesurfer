...
MRI *MRIconvolve1d(MRI *mri_src, MRI *mri_dst, float *k, int len, int axis, int src_frame, int dst_frame)
{
  ...
  switch (mri_src->type) {
    ...
    shared(depth, height, width, len, halflen, mri_src, mri_dst, src_frame, dst_frame, xi, yi, zi) schedule(dynamic, 1)
#endif
          for (z = 0; z < depth; z++) {
	    ROMP_PFLB_begin
    ...
    shared(depth, height, width, len, halflen, mri_dst, src_frame, dst_frame, xi, yi, zi) schedule(dynamic, 1)
#endif
          for (z = 0; z < depth; z++) {
	    ROMP_PFLB_begin
    ...
    MRI *mri_intensity, MRI *mri_label, MRI *mri_smooth, int niter, int label, int fixed_label, float min_change)
{
  ...
  for (i = 0; i < niter; i++) {
    ...
    ROMP_PF_begin
#ifdef HAVE_OPENMP
    #pragma omp parallel for if_ROMP(experimental) firstprivate(max_change) shared(mri_tmp, mri_label, Gx, Gy, Gz) schedule(dynamic, 1)
#endif
    for (x = 0; x < mri_tmp->width; x++) {
      ROMP_PFLB_begin
