...
double CostFunctions::mean(MRI *mri, int frame)
{
  ...
  int z;
#ifdef HAVE_OPENMP
#pragma omp parallel for schedule(dynamic)reduction(+ : d)
#endif
...
double CostFunctions::mean(MRI *mri, const vnl_matrix_fixed< double, 4, 4 > &Mi, int frame, int d1, int d2, int d3)
{
...
#pragma omp atomic
  ...
  unsigned int n =
      (((xmax - xmin - 1) / d1) + 1) * (((ymax - ymin - 1) / d2) + 1) * (((zmax - zmin - 1) / d3) + 1) - ocount;
  assert(n == count);
  return d / n;
}
...
double CostFunctions::leastSquares(MRI *mriS,
                                   MRI *mriT,
                                   const vnl_matrix_fixed< double, 4, 4 > &Msi,
...
double CostFunctions::absDiff(MRI *mriS,
                              MRI *mriT,
                              const vnl_matrix_fixed< double, 4, 4 > &Msi,
                              const vnl_matrix_fixed< double, 4, 4 > &Mti,
                              int d1,
...
double CostFunctions::leastSquares(MRI_BSPLINE *mriS,
                                   MRI_BSPLINE *mriT,
                                   const vnl_matrix_fixed< double, 4, 4 > &Msi,
                                   const vnl_matrix_fixed< double, 4, 4 > &Mti,
                                   int d1,
                                   ...
                                   int d3)
{
  ...
  double dt[4] = {(double)mriT->coeff->width,
                  (double)mriT->coeff->height,
                  (double)mriT->coeff->depth,
                  (double)mriT->coeff->nframes};
  int x, y, z, f;
  double dd, d = 0;
  double xs, ys, zs;
  double xt, yt, zt;
...
double CostFunctions::localNCC(MRI *mriS,
                               MRI *mriT,
                               const vnl_matrix_fixed< double, 4, 4 > &Msi,
                               const vnl_matrix_fixed< double, 4, 4 > &Mti,
                               int d1,
...
MRI *CostFunctions::mapMRI(MRI *mri, const vnl_matrix_fixed< double, 4, 4 > &Mi, int d1, int d2, int d3)
{
...
double CostFunctions::NCC(MRI *mriS,
                          MRI *mriT,
                          const vnl_matrix_fixed< double, 4, 4 > &Msi,
                          const vnl_matrix_fixed< double, 4, 4 > &Mti,
                          int d1,
...
double CostFunctions::tukeyBiweight(MRI *i1, MRI *i2, double sat)
{
  ...
  if (i2 == NULL) {
    MRIiterator it1(i1);
    for (it1.begin(); !it1.isEnd(); it1++) diff[cc] = (*it1);
    cc++;
  }
  else {
    MRIiterator it1(i1);
  ...
  double d = 0;
  for (int i = 0; i < n; i++) d += rhoTukeyBiweight(diff[i] / sigma, sat);
...
double CostFunctions::tukeyBiweight(MRI *mriS,
                                    MRI *mriT,
                                    const vnl_matrix_fixed< double, 4, 4 > &Msi,
                                    const vnl_matrix_fixed< double, 4, 4 > &Mti,
                                    int d1,
...
double CostFunctions::SBCost(MRI *mriS,
                             MRI *mriT,
                             const vnl_matrix_fixed< double, 4, 4 > &Msi,
                             const vnl_matrix_fixed< double, 4, 4 > &Mti,
                             int d1,
                             ...
                             int d3)
{
  ...
  sort(idx.begin(), idx.end(), [&iv1](size_t i1, size_t i2) { return iv1[i1] > iv1[i2]; });
  ...
  for (int d = 0; d < i->depth; d++)
    for (int h = 0; h < i->height; h++)
      for (int w = 0; w < i->width; w++) {
        dd += pow(d + 1.0, z) * pow(h + 1.0, y) * pow(w + 1.0, x) * MRIgetVoxVal(i, (int)w, (int)h, (int)d, 0);
      }
  return dd;
}
...
// M_ijk below is the moment(i,j,k)
{
  ...
  for (uint i = 0; i < 3; i++) evec.set_column(i, SymEig.V.get_column(largetosmall[i]));
  ...
  if (d < 0) {
    ...
    for (int r = 0; r < 3; r++) evec[r][0] = -evec[r][0];
