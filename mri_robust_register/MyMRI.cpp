      ...
      for (x = 0; x < w; x++) {
        if (randpos >= 0) {
          ...
          for (f = 0; f < mri_src->nframes; f++)
            MRIsetVoxVal(mri_dst, x, y, z, f, MRIgetVoxVal(mri_src, 2 * x + dx, 2 * y + dy, 2 * z + dz, f));
        }
        else
          for (f = 0; f < mri_src->nframes; f++)
            MRIsetVoxVal(mri_dst, x, y, z, f, MRIgetVoxVal(mri_src, 2 * x, 2 * y, 2 * z, f));
      }
...
bool MyMRI::getPartials(MRI *mri, MRI *&outfx, MRI *&outfy, MRI *&outfz, MRI *&outblur)
{
  ...
  for (int f = 0; f < mri->nframes; f++) {
    ...
    if (!is2d) outfz = MRIconvolve1d(mbxby, outfz, &MRIFvox(mri_derfilter, 0, 0, 0), klen, whd[3 - 1], 0, f);
...
//  bool conform makes cube image (w=h=d) and adjust to min of 256
{
  ...
  // cout << " zsize: " << zsize << " depth: " << mri->depth << " fdepth: " << fdepth << " new depth: " << ret[2] <<
  // endl;
...
{
  return (mri->xsize == 1 && mri->ysize == 1 && mri->zsize == 1 && mri->width == 256 && mri->height == 256 &&
          mri->depth == 256);
}
                                                                ...
                                                                const vnl_matrix_fixed< double, 4, 4 > &m_vox)
{
  ...
  vnl_matrix_fixed< double, 4, 4 > m_ras =
      MyMatrix::convertMATRIX2VNL(m_voxel_to_ras) * m_vox * MyMatrix::convertMATRIX2VNL(m_ras_to_voxel);
...
MRI *MyMRI::gaussianCube(int size)
{
  ...
  for (z = 0; z < size; z++)
    for (y = 0; y < size; y++)
      for (x = 0; x < size; x++) MRIsetVoxVal(g, x, y, z, 0, MRIgetVoxVal(g, x, y, z, 0) / (float)sum);
...
// expects uchar image between 0..(nbins-1) intensity values
{
  ...
  double histo[nbins];
  for (int o = 0; o < nbins; o++) histo[o] = 0.0f;
  ...
  for (int o = 0; o < nbins; o++) alltotal += histo[o];
...
#ifdef HAVE_OPENMP
#pragma omp parallel for firstprivate(y, x, val, zz, yy, xx, zzz, yyy, xxx, d, wij, count) \
    shared(depth, height, width, radius, nrad, prad, mri, nlsdI, nsize, sigma2) schedule(dynamic, 1)
#endif
  for (z = 0; z < depth; z++) {
    if (z % 10 == 0) {
      cout << "." << flush;
                ...
                for (yyy = -prad; yyy <= prad; yyy++)
                  for (xxx = -prad; xxx <= prad; xxx++) {
                    d = MRIgetVoxVal(mri, x + xxx, y + yyy, z + zzz, 0) -
                        MRIgetVoxVal(mri, x + xx + xxx, y + yy + yyy, z + zz + zzz, 0);
                    wij += d * d;
                  }
              wij = exp(-wij / sigma2);
              weights[count] = wij;
...
// shared(depth,height,width,nxoffset,nyoffset,nzoffset,nlsdI) schedule(dynamic,1) #endif
...
MRI *MyMRI::entropyImage(MRI *mri, int radius, bool ball, bool correction, MRI *mask)
{
  ...
  if (mriuchar != mri) MRIfree(&mriuchar);
  if (localMask) MRIfree(&localMask);
...
#ifdef HAVE_OPENMP
#pragma omp parallel for firstprivate(y, x, o, histo, count, zz, yy, xx, pos, histosum, entropy, etmp) \
    shared(depth, height, width, radius, entI, nbins, ssize, xmax, ymax, zmax, mriIn, g) schedule(dynamic, 1)
#endif
  for (z = 0; z < depth; z++) {
    // if ((z+1)%zinterval == 0) cout << (z+1)/zinterval << flush;
    // else  if ((z+1)%zinterval2 ==0) cout << "." << flush;
    ...
    for (y = 0; y < height; y++) {
      for (x = 0; x < width; x++) {
        ...
        // init histo
        for (o = 0; o < nbins; o++) histo[o] = 0.0f;
        ...
        histosum = 0.0;
        for (o = 0; o < nbins; o++) histosum += histo[o];
...
void MyMRI::get3Dcorrection(
    double *histo, unsigned int v1, unsigned int v2, unsigned int v3, unsigned int v4, unsigned int intRange)
{
  ...
  if (a == 0 && b == 0 && c == 0) {
    ...
    if (I3 != I2) {
      ...
      for (i = 0; i < vecLen; i++) tempVec[i] = I3 + i;
      tempVec[0] = tempVec[0] + 0.25;
      tempVec[vecLen - 1] = tempVec[vecLen - 1] - 0.25;

      double PDFvec[vecLen];
      for (i = 0; i < vecLen; i++) PDFvec[i] = 3.0f / a * (tempVec[i] - I3) / b * (tempVec[i] - I3) / c;
      PDFvec[0] = 0.5 * PDFvec[0];
      PDFvec[vecLen - 1] = 0.5 * PDFvec[vecLen - 1];

      for (i = I3; i <= I2; i++) tempPDF[i] = PDFvec[i - I3];
    }

    if (I2 != I1) {
      vecLen = I1 - I2 + 1;
      vector< double > tempVec(vecLen, 0.0);
      for (i = 0; i < vecLen; i++) tempVec[i] = I2 + i;
      tempVec[0] = tempVec[0] + 0.25;
      tempVec[vecLen - 1] = tempVec[vecLen - 1] - 0.25;

      double PDFvec[vecLen];
      for (i = 0; i < vecLen; i++)
        PDFvec[i] = (3.0f / a) * ((((I1 - tempVec[i]) / (b - c)) * ((tempVec[i] - I3) / b)) +
                                  (((I0 - tempVec[i]) / (a - c)) * ((tempVec[i] - I2) / (b - c))));
      PDFvec[0] = 0.5 * PDFvec[0];
      PDFvec[vecLen - 1] = 0.5 * PDFvec[vecLen - 1];

      for (i = I2; i <= I1; i++) tempPDF[i] = PDFvec[i - I2];
    }

    if (I1 != I0) {
      vecLen = I0 - I1 + 1;
      vector< double > tempVec(vecLen, 0.0);
      for (i = 0; i < vecLen; i++) tempVec[i] = I1 + i;
      tempVec[0] = tempVec[0] + 0.25;
      tempVec[vecLen - 1] = tempVec[vecLen - 1] - 0.25;

      double PDFvec[vecLen];
      for (i = 0; i < vecLen; i++)
        PDFvec[i] = (3.0f / a) * (((I0 - tempVec[i]) / (a - c)) * ((I0 - tempVec[i]) / (a - b)));
      PDFvec[0] = 0.5 * PDFvec[0];
      PDFvec[vecLen - 1] = 0.5 * PDFvec[vecLen - 1];

      for (i = I1; i <= I0; i++) tempPDF[i] = PDFvec[i - I1];
    }

    double sum = 0;
    for (i = 0; i < intRange; i++) sum += tempPDF[i];
    for (i = 0; i < intRange; i++) tempPDF[i] = tempPDF[i] / sum;
  }

  for (i = 0; i < intRange; i++) histo[i] = histo[i] + tempPDF[i];
}
...
MRI *MyMRI::getNormalizedImage(MRI *mri)
{
  ...
  for (int f = 0; f < mri->nframes; f++) {
    ...
    int y;
#ifdef HAVE_OPENMP
#pragma omp parallel for schedule(dynamic) reduction(+ : mean)
#endif
    for (y = 0; y < mri->height; y++) {
      int z, x;
      double v;
    ...
    double std = 0.0;
#ifdef HAVE_OPENMP
#pragma omp parallel for schedule(dynamic) reduction(+ : std)
#endif
    for (y = 0; y < mri->height; y++) {
      int z, x;
      double v;
